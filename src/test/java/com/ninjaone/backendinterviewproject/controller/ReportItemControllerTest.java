...
public class ReportItemControllerTest {
    ...
    public void givenNoContextWhenRequestingTheTotalCalculationThenReturnTheValue() throws Exception {
        when(service.getTotalAmount()).thenReturn(new Report(0.0, Collections.emptyList()));
        mockMvc.perform(get("/total"))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(content().string(objectMapper.writeValueAsString(new Report(0.0, Collections.emptyList()))));
    }
}
